...
dev.off()
```
#Looking at the dispersion/variance of samples using perm_disp prior to sample removal
``` {r perm_disp prior to sample removal}
condition_por <- colData(gdds_por)$eatenvscontrol

# Run the PERMDISP test using the distance matrix and condition
permdisp_result_por <- betadisper(gsampleDists_por, condition_por)

# View the summary of PERMDISP result
summary(permdisp_result_por)
# Step 2: Perform an ANOVA on the betadisper result
permdisp_anova_por <- anova(permdisp_result_por)

# Step 3: Extract the p-value from the ANOVA result
p_value_por <- permdisp_anova_por$`Pr(>F)`[1]

# Step 4: Print the p-value
print(p_value_por)
# 0.003650602 = significant difference

# Plot PERMDISP result (dispersion by group)
boxplot(permdisp_result_por, main = "PERMDISP: Group Dispersion")

# Optionally, use a scatter plot to see dispersion
plot(permdisp_result_por, main = "PERMDISP - Group Dispersions")
```
...
write.csv(gcount_filt_por2, "data/all_genes_por.csv", row.names = TRUE)
```
...
#Looking at the dispersion/variance of samples using perm_disp
``` {r perm_disp}
condition_por2 <- colData(gdds_por2)$eatenvscontrol

# Run the PERMDISP test using the distance matrix and condition
permdisp_result_por2 <- betadisper(gsampleDists_por2, condition_por2)

# View the summary of PERMDISP result
summary(permdisp_result_por2)
# Step 2: Perform an ANOVA on the betadisper result
permdisp_anova_por2 <- anova(permdisp_result_por2)

# Step 3: Extract the p-value from the ANOVA result
p_value_por2 <- permdisp_anova_por2$`Pr(>F)`[1]

# Step 4: Print the p-value
print(p_value_por2)
# 0.2726992 = no significant difference

# Plot PERMDISP result (dispersion by group)
boxplot(permdisp_result_por2, main = "PERMDISP: Group Dispersion")

# Optionally, use a scatter plot to see dispersion
plot(permdisp_result_por2, main = "PERMDISP - Group Dispersions")
```
Since removing samples that looked like outliers, the p-value of the variance between samples has changed from signficant at the beginning (all samples included) to non-signficant (removing 225R, 34R, 43R and 16R). Chat to Ariana about whether this changes anything.
...
gPCAdata_por2 <- plotPCA(gvst_por2, ntop = nrow(gvst_por2), intgroup = c("eatenvscontrol"), returnData=TRUE)
...
allgenesfilt_PCA_por2 <- ggplot(gPCAdata_por2, aes(PC1, PC2, color=eatenvscontrol)) + 
  ...
  ylab(paste0("PC2: ",percentVar_por2[2],"% variance")) +
  scale_color_manual(name="Eaten vs Control", values=c("control"="gray", "eaten"="orange"))+
  annotate(
    "text",
    x = 0,
    y = -20,
    label = "p[eatenvscontrol]=0.57",
    color = "black"
  ) +
  theme_classic() + #Set background color
  theme(panel.border = element_blank(), # Set border
...
ggsave(
  "figures/porites/pca_por_ellipse_2.svg",
  allgenes_ellipse_por2,
  width = 9,
  height = 5
)
```
...
dim(eatenvscontrol_por2) # 18 DEGs
...
``` {r Create gene_counts_long_por}
#Create gene_counts_long
gene_counts_por2 <- counts(DEG_T_por2, normalized = TRUE)
gene_counts_df_por2 <- as.data.frame(gene_counts_por2)
gene_counts_df_por2$gene_id <- rownames(gene_counts_df_por2)
library(tidyr)
gene_counts_long_por2 <- pivot_longer(
  gene_counts_df_por2,
  cols = -gene_id,
  names_to = "sample",
  values_to = "count"
...
``` {r Create counts_annotated_por}
# Turn into dataframe and add gene IDs
de_df_por2 <- as.data.frame(eatenvscontrol_por2)
de_df_por2$gene_id <- rownames(de_df_por2)
# View or filter DE genes
head(de_df_por2)
sig_genes_por2 <- de_df_por2[which(de_df_por2$padj < 0.05), ]
#Create dataframe of DEGs with sample info too
norm_counts_por2 <- counts(DEG_T_por2, normalized = TRUE)
norm_counts_df_por2 <- as.data.frame(norm_counts_por2)
norm_counts_df_por2$gene_id <- rownames(norm_counts_df_por2)
library(tidyr)
counts_long_por2 <- pivot_longer(norm_counts_df_por2, 
                             cols = -gene_id,
                             names_to = "sample",
                             values_to = "count")
sample_meta_por2 <- as.data.frame(colData(DEG_T_por2))
sample_meta_por2$sample <- rownames(sample_meta_por2)
library(dplyr)
counts_annotated_por2 <- left_join(counts_long_por2, sample_meta_por2, by = "sample")
```
...
DEG_por2_eatenvscontrol_heatmap <- pheatmap(assay(DEG_eatenvscontrol_vst_por2), cluster_rows=TRUE, show_rownames=TRUE, color=inferno(10), show_colnames=TRUE, fontsize_row=6, scale="row", cluster_cols=TRUE, annotation_col=df_por2, annotation_colors = ann_colors_por, labels_col=legend_names_col_por)

save_pheatmap_pdf(DEG_por2_eatenvscontrol_heatmap, "figures/porites/DEG_por2_eatenvscontrol_heatmap.pdf", width=15, height=20)
dev.off()

ggsave(
  "figures/porites/DEG_por2_eatenvscontrol_heatmap.svg",
  DEG_por2_eatenvscontrol_heatmap,
  width = 9,
  height = 5
)
```
...
# Plot PCA of DEGs
```{r Plot a PCA of DEGs}
# Get the gene IDs of the significant genes
sig_gene_ids_por <- sig_genes_por2$gene

# rlog transformation (can also use vst for variance-stabilizing)
rld_por <- vst(DEG_T_por2)

# Get the transformed counts
rld_mat_por <- assay(rld_por)

# Subset the transformed counts to include only significant genes
rld_sig_genes_por <- rld_mat_por[sig_gene_ids_por, ]
# Run PCA (t() is important to transpose for PCA, where rows are samples)
pca_por <- prcomp(t(rld_sig_genes_por), scale. = TRUE)

percentVar_pordeg <- pca_por$sdev^2 / sum(pca_por$sdev^2)

# Convert to percentages
percentVar_pordeg <- round(percentVar_pordeg * 100, 1)

percentVar_pordeg
#PC1 # 54.8 and PC2 = 18.7

# Create a dataframe with PCA results
pca_df_por <- as.data.frame(pca_por$x)
pca_df_por$sample <- rownames(pca_df_por)
sample_meta_por2 <- as.data.frame(colData(DEG_T_por2))
sample_meta_por2$sample <- rownames(sample_meta_por2)

# Merge PCA results with sample metadata
pca_df_por <- left_join(pca_df_por, sample_meta_por2, by = "sample")
por_pca_deg <- ggplot(pca_df_por, aes(x = PC1, y = PC2, colour = eatenvscontrol)) +
  geom_point(size = 3) +
  geom_text_repel(
  aes(label = sample),
  box.padding = 0.5,
  ...
  labs(title = "PCA of Porites DEGs (padj < 0.05)", x = "PC1", y = "PC2") +
  scale_color_manual(name="Eaten vs Control", values=c("control"="gray", "eaten"="orange")) +
  theme_minimal()
plot(por_pca_deg)

ggsave("figures/porites/por_pca_deg.pdf", por_pca_deg, width=8, height=4)
ggsave("figures/porites/por_pca_deg.jpeg", por_pca_deg, width=8, height=4)

#Add a plot with elipses.  
por_pca_deg_ellipse<-por_pca_deg + stat_ellipse();por_pca_deg_ellipse
ggsave("figures/porites/por_pca_deg_ellipse.pdf", por_pca_deg_ellipse, width=9, height=5)
ggsave("figures/porites/por_pca_deg_ellipse.jpeg", por_pca_deg_ellipse, width=9, height=5)

ggsave(
  "figures/porites/por_pca_deg_ellipse2.svg",
  por_pca_deg_ellipse,
  width = 9,
  height = 5
)
...
```{r, expression counts of genes}
# Matrix of VST expression values
vst_all_por <- assay(rld_por)

# Data frame for export
vst_all_por_df <- cbind(
  gene = rownames(vst_all_por),
  as.data.frame(vst_all_por)
)

write.csv(vst_all_por_df,
          "data/porites_all_genes_vst_expression.csv",
          row.names = FALSE)
```