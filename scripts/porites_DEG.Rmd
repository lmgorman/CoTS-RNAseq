...
save_pheatmap_pdf(pht, "data/plots/sample_distances_por.pdf")
dev.off()
```
...
ggsave("data/plots/pca_por.pdf", allgenesfilt_PCA_por, width=8, height=4)
ggsave("data/plots/pca_por.jpeg", allgenesfilt_PCA_por, width=8, height=4)

#Add a plot with elipses.  
allgenes_ellipse_por<-allgenesfilt_PCA_por + stat_ellipse();allgenes_ellipse_por
ggsave("data/plots/pca_ellipse_por.pdf", allgenes_ellipse_por, width=9, height=5)
ggsave("data/plots/pca_ellipse_por.jpeg", allgenes_ellipse_por, width=9, height=5)
```
...
nrow(gcount_filt_por2) #After
    #[1] 24301
```
Now has around 8,000 more genes after removing samples
...
save_pheatmap_pdf(pht_por2, "data/plots/sample_distances_por2.pdf")
dev.off()
```
Diagnol = great
...
ggsave("data/plots/pca_por2.pdf", allgenesfilt_PCA_por2, width=8, height=4)
ggsave("data/plots/pca_por2.jpeg", allgenesfilt_PCA_por2, width=8, height=4)

#Add a plot with elipses.  
allgenes_ellipse_por2<-allgenesfilt_PCA_por2 + stat_ellipse();allgenes_ellipse_por2
ggsave("data/plots/pca_ellipse_por2.pdf", allgenes_ellipse_por2, width=9, height=5)
ggsave("data/plots/pca_ellipse_por2.jpeg", allgenes_ellipse_por2, width=9, height=5)
```
...
```{r}
DEG_eatenvscontrol_por2_vol <- DESeq(gdds_por2)
#Eaten vs control
res <- results(DEG_eatenvscontrol_por2_vol, contrast=c("eatenvscontrol","control","eaten"))

volcano_por2_eatenvscontrol<-EnhancedVolcano(res,
                                     lab = rownames(res),
                                     x = 'log2FoldChange',
                                     y = 'pvalue',
                                     title = 'Porites DEGS eaten by CoTS vs control',
                                     pCutoff = 0.01,
                                     FCcutoff = 1.5,
                                     pointSize = 3.0,
                                     labSize = 0.0, 
...
ggsave("data/plots/volcano_por2_eatenvscontrol.pdf", plot=volcano_por2_eatenvscontrol, width=18, height=6)
ggsave("data/plots/volcano_por2_eatenvscontrol.png", plot=volcano_por2_eatenvscontrol, width=18, height=6)
```
...
save_pheatmap_pdf(DEG_por2_eatenvscontrol_heatmap, "data/plots/DEG_por2_eatenvscontrol_heatmap.pdf", width=15, height=20)
dev.off()
```