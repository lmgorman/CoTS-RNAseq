#!/bin/bash
#SBATCH --job-name=STAR_genome_index_Ahya
#SBATCH --nodes=1
#SBATCH --cpus-per-task=8
#SBATCH --mem=250G
#SBATCH -p uri-cpu
#SBATCH --time=24:00:00
#SBATCH -o build_star_index_jaAcrHyac4.1-%j.out
#SBATCH -e build_star_index_jaAcrHyac4.1-%j.error
#SBATCH -D /work/pi_hputnam_uri_edu/ashuffmyer/cots-gorman/acr_hya_jaAcrHyac4.1

# Load modules
module load uri/main
module load STAR/2.7.11b-GCC-12.3.0
module load gffread/0.12.7

echo "[$(date)] Converting GFF3 to GTF"
gffread /work/pi_hputnam_uri_edu/ashuffmyer/cots-gorman/GCA_964291705.1_jaAcrHyac4.1_genomic.gbff.gff \
  -T -o GCA_964291705.1_jaAcrHyac4.1_genomic.gtf

echo "[$(date)] Starting STAR genome index build in scratch: $SCRATCHDIR"

# Set custom scratch directory
SCRATCHDIR=/scratch3/workspace/lucy_gorman_uri_edu-lucyscratch/star_index_jaAcrHyac4.1

mkdir -p "$SCRATCHDIR"

# Copy genome FASTA and GTF to scratch
echo "[$(date)] Copying genome FASTA and GTF to scratch"
cp /work/pi_hputnam_uri_edu/ashuffmyer/cots-gorman/acr_hya_jaAcrHyac4.1/GCA_964291705.1_jaAcrHyac4.1_genomic.fna "$SCRATCHDIR"
cp GCA_964291705.1_jaAcrHyac4.1_genomic.gtf "$SCRATCHDIR"

# Set variables to point to scratch versions
GENOME_FA="$SCRATCHDIR/GCA_964291705.1_jaAcrHyac4.1_genomic.fna"
GTF_FILE="$SCRATCHDIR/GCA_964291705.1_jaAcrHyac4.1_genomic.gtf"
INDEX_DIR="$SCRATCHDIR"

# Run STAR genomeGenerate
STAR --runThreadN 8 \
    --runMode genomeGenerate \
    --genomeDir "$INDEX_DIR" \
    --genomeFastaFiles "$GENOME_FA" \
    --sjdbGTFfile "$GTF_FILE" \
    --genomeSAindexNbases 13 \
    --sjdbOverhang 99

# Check if STAR index was created before copying
FINAL_OUTPUT_DIR="/work/pi_hputnam_uri_edu/ashuffmyer/cots-gorman/star_index_jaAcrHyac4.1"
mkdir -p "$FINAL_OUTPUT_DIR"

if [ -f "$INDEX_DIR/Genome" ]; then
    cp -r "$INDEX_DIR"/* "$FINAL_OUTPUT_DIR"
    echo "[$(date)] Genome index build completed and copied to $FINAL_OUTPUT_DIR"
else
    echo "[$(date)] ERROR: STAR index not created. Check STAR logs." >&2
    exit 1
fi

