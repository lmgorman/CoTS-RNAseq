...
all_genes_acr<-read.csv("output/allgenes_acr.csv")
```

2. Read in file of DEG's between eaten and control
```{r}
acr_eatenvscontrol_DEG_list<-read.csv("data/DEG_acr.csv")
acr_eatenvscontrol_DEG_counts<-read.csv("X.csv")
acr_eatenvscontrol_DEG_list$contrast<-factor(acr_eatenvscontrol_DEG_list$contrast, levels=c("eaten", "control"))
```

# Read in annotation files 
Genome used for RNA-seq - https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_020536085.1/ = no functional enrichment annotations so will need to run this ourselves 