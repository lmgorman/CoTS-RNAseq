...
```{r}
if ("tidyversea" %in% rownames(installed.packages()) == 'FALSE') install.packages('tidyverse') 
...
write.csv(filtered_genes_por,file='data/Por_annot_filtered.csv') #22,214 genes we found in our RNA gene expression dataset have annotations
```
...
```{r}
geneID2GO_por<-filtered_genes_por%>%dplyr::select(Sequence.Name, Annotation.GO.ID)
geneID2GO_por <- as.data.frame(geneID2GO_por)
background_por <- geneID2GO_por%>%pull("Sequence.Name")
...
geneID2GO_por <- split(as.character(geneID2GO_por$Annotation.GO.ID), geneID2GO_por$Sequence.Name)
#so now our geneID2GO_por object has all our genes in our dataset that have GO terms
```

Run TopGO

```{r}
por_test_set<-por_eatenvscontrol_DEG_list%>%pull(gene) #this is our DEG genes
...
geneList_por <- factor(background_por %in% por_test_set, levels = c(FALSE, TRUE)) # 4. Check structure
str(geneList_por)
#Make sure the geneList_por is an integer NOT a logical
geneList_por <- as.integer(background_por %in% por_test_set) #this is how many of our DEGs have annotations..
sum(geneList_por) #18 DEGs have GO terms = makes sense as I only have 18 DEGs
names(geneList_por) <- background_por
table(geneList_por) 

str(geneID2GO_por, max.level = 1)
head(geneID2GO_por, 3)
sum(names(geneList_por) %in% names(geneID2GO_por))# 17861
...
simMatrix_por <- rrvgo::calculateSimMatrix(p05_res_por$GO.ID,
...
```{r}
library("seqinr")
por_fasta_sequences <- read.fasta(file = "data/Porites_evermanni_v1.annot.pep.fa", seqtype = "AA", as.string = TRUE)
# Convert to data frame
por_fasta_sequences <- data.frame(id = names(por_fasta_sequences), sequence = unlist(por_fasta_sequences))
selected_fasta <- por_fasta_sequences[por_fasta_sequences$id %in% por_eatenvscontrol_DEG_list$gene, ]
filtered_annot <- Por_annot[Por_annot$Sequence.Name %in% selected_fasta$id, ]
Porites_DEGs_metadata <- merge(selected_fasta, filtered_annot, by.x = "id", by.y = "Sequence.Name")
#Remove multiple semi-colons
Porites_DEGs_metadata <- Porites_DEGs_metadata %>%
  mutate(across(where(is.character), ~ gsub(";+", "", .)))
#Write file for further analysis
write.csv(Porites_DEGs_metadata,file='data/Porites_DEGs_metadata.csv') 
#Write file of just DEG amino acids for BLASTing against my venom databases
# Turn the dataframe into a named list
Por_sequences <- AAStringSet(selected_fasta$sequence)
names(Por_sequences) <- selected_fasta$id
writeXStringSet(Por_sequences, filepath = "data/Porites_DEGs.fasta")

#Do same for all genes
library(sequinr)
library(dplyr)

# 1. Read protein FASTA
por_fasta_sequences <- read.fasta(
  file = "data/Porites_evermanni_v1.annot.pep.fa",
  seqtype = "AA",
  as.string = TRUE
)

# Convert FASTA list into a dataframe
por_fasta_df <- data.frame(
  id = names(por_fasta_sequences),
  sequence = unlist(por_fasta_sequences),
  stringsAsFactors = FALSE
)

# 2. Subset FASTA sequences for your filtered gene list
# Replace `gene` with the name of your gene column in filtered_genes_por
selected_fasta <- por_fasta_df %>%
  filter(id %in% filtered_genes_por$gene)

# 3. (Optional) Filter annotation table
# Only run this if you have Por_annot loaded and want annotation info
filtered_annot <- Por_annot %>%
  filter(Sequence.Name %in% selected_fasta$id)

# 4. Merge FASTA sequences + annotation (optional)
# If you do NOT have annotation, remove this and just keep selected_fasta
Porites_metadata <- merge(
  selected_fasta,
  filtered_annot,
  by.x = "id",
  by.y = "Sequence.Name",
...
# 5. Clean up repeated semicolons in character fields
Porites_metadata <- Porites_metadata %>%
  mutate(across(where(is.character), ~ gsub(";+", "", .)))

# 6. Write output
write.csv(Porites_metadata, file = "data/all_expressed_Porites.csv", row.names = FALSE)
```