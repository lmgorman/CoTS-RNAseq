...
``` {r perm_disp}
condition <- colData(gdds_2)$eatenvscontrol

# Run the PERMDISP test using the distance matrix and condition
permdisp_result <- betadisper(gsampleDists_2, condition)

# View the summary of PERMDISP result
summary(permdisp_result)
# Step 2: Perform an ANOVA on the betadisper result
permdisp_anova <- anova(permdisp_result)

# Step 3: Extract the p-value from the ANOVA result
p_value <- permdisp_anova$`Pr(>F)`[1]

# Step 4: Print the p-value
print(p_value)

# Plot PERMDISP result (dispersion by group)
boxplot(permdisp_result, main = "PERMDISP: Group Dispersion")

# Optionally, use a scatter plot to see dispersion
plot(permdisp_result, main = "PERMDISP - Group Dispersions")
```
Lots of dispersion for the eaten samples - unsuprising. ANOVA p-value = 0.00804436 which shows there is a significant dispersion due to the treatments (eaten vs control)
...
ggsave(
  "figures/acropora/pca_acr_ellipse_2.svg",
  allgenes_ellipse_2,
  width = 9,
  height = 5
)
...
gene_names <- rownames(DEG_T)
sample_names <- colData(DEG_T) 
eatenvscontrol <- as.data.frame(eatenvscontrol)
eatenvscontrol <- eatenvscontrol %>%
  mutate(gene_ID = rownames(eatenvscontrol))
eatenvscontrol <- subset(eatenvscontrol, padj<0.05)
eatenvscontrol$contrast <- c("eatenvscontrol")
dim(eatenvscontrol)
write.csv(eatenvscontrol,"data/DEG_acr.csv", row.names = FALSE)
```

``` {r Create gene_counts_long}
#Create gene_counts_long
gene_counts <- counts(DEG_T, normalized = TRUE)
gene_counts_df <- as.data.frame(gene_counts)
gene_counts_df$gene_id <- rownames(gene_counts_df)
library(tidyr)
gene_counts_long <- pivot_longer(
  gene_counts_df,
  cols = -gene_id,
  names_to = "sample",
  values_to = "count"
...
``` {r Create counts_annotated}
# Create counts_annotated
eatenvscontrol <- results(DEG_T, contrast = c("eatenvscontrol", "control", "eaten"))
# Turn into dataframe and add gene IDs
de_df <- as.data.frame(eatenvscontrol)
de_df$gene_id <- rownames(de_df)
# View or filter DE genes
head(de_df)
sig_genes <- de_df[which(de_df$padj < 0.05), ]
#Create dataframe of DEGs with sample info too
norm_counts <- counts(DEG_T, normalized = TRUE)
norm_counts_df <- as.data.frame(norm_counts)
norm_counts_df$gene_id <- rownames(norm_counts_df)
library(tidyr)
counts_long <- pivot_longer(norm_counts_df, 
                             cols = -gene_id,
                             names_to = "sample",
                             values_to = "count")
sample_meta <- as.data.frame(colData(DEG_T))
sample_meta$sample <- rownames(sample_meta)
library(dplyr)
counts_annotated <- left_join(counts_long, sample_meta, by = "sample")
```
...
length(unique(DEG_eatenvscontrol$gene_ID))

#Get a VST normalized gene count matrix for these DEGs.This isn't knitting correctly so blocked out. 
...
deg_genes <- rownames(DEG_eatenvscontrol)

DEG_eatenvscontrol_vst <- gdds_2[deg_genes, ]

DEG_eatenvscontrol_vst <- varianceStabilizingTransformation(
  DEG_eatenvscontrol_vst,
  blind = FALSE
)

DEG_sig <- DEG_eatenvscontrol[
  !is.na(DEG_eatenvscontrol$padj) &
  DEG_eatenvscontrol$padj < 0.05, 
]
...
DEG_acr_eatenvscontrol_heatmap <- pheatmap(
  assay(DEG_eatenvscontrol_vst),
  cluster_rows = TRUE,
  show_rownames = TRUE,
  color = viridis::inferno(10),
  scale = "row",
  cluster_cols = TRUE,
  annotation_col = df_acr,
  annotation_colors = ann_colors_acr
)
...
vst_top <- DEG_eatenvscontrol_vst[rownames(top_DEGs), ]

DEG_acr_eatenvscontrol_heatmap_top50 <- pheatmap(
  assay(vst_top),
  cluster_rows = TRUE,
  show_rownames = TRUE,
  fontsize_row = 4,
...
save_pheatmap_pdf(DEG_acr_eatenvscontrol_heatmap, "figures/porites/DEG_acr_eatenvscontrol_heatmap.pdf", width=15, height=20)
dev.off()

ggsave(
  "figures/acropora/DEG_acr_eatenvscontrol_heatmap.svg",
  DEG_acr_eatenvscontrol_heatmap,
  width = 9,
  height = 5
)

ggsave(
  "figures/acropora/DEG_acr_eatenvscontrol_heatmap_top50.svg",
  DEG_acr_eatenvscontrol_heatmap_top50,
  width = 9,
  height = 5
)
```
...
#Previous chunk not working so had to block out this one too since it uses object computed in previous chunk
#Set themes and metadata.  
...
sig_genes <- eatenvscontrol[which(eatenvscontrol$padj < 0.05), ]
# Get the gene IDs of the significant genes
sig_gene_ids <- rownames(sig_genes)
# Subset the normalized counts to keep only significant genes
sig_counts <- norm_counts[sig_gene_ids, ]
...
rld_sig_genes <- rld_mat[sig_gene_ids, ]
# Run PCA (t() is important to transpose for PCA, where rows are samples)
pca <- prcomp(t(rld_sig_genes), scale. = TRUE)

percentVar_acrdeg <- pca$sdev^2 / sum(pca$sdev^2)

# Convert to percentages
percentVar_acrdeg <- round(percentVar_acrdeg * 100, 1)

percentVar_acrdeg
#PC1 # 61.0 and PC2 = 9.0
...
acr_pca_deg <- ggplot(pca_df, aes(x = PC1, y = PC2, colour = eatenvscontrol)) +
  geom_point(size = 3) +
  geom_text_repel(
  aes(label = sample),
  box.padding = 0.5,
  ...
  labs(title = "PCA of Acropora DEGs (padj < 0.05)", x = "PC1", y = "PC2") +
  scale_color_manual(name="Eaten vs Control", values=c("control"="gray", "eaten"="orange")) +
  theme_minimal()
plot(acr_pca_deg)

ggsave("figures/acropora/acr_pca_deg.pdf", acr_pca_deg, width=8, height=4)
ggsave("figures/acropora/acr_pca_deg.jpeg", acr_pca_deg, width=8, height=4)

#Add a plot with elipses.  
acr_pca_deg_ellipse<-acr_pca_deg + stat_ellipse();acr_pca_deg_ellipse
ggsave("figures/acropora/acr_pca_deg_ellipse.pdf", acr_pca_deg_ellipse, width=9, height=5)
ggsave("figures/acropora/acr_pca_deg_ellipse.jpeg", acr_pca_deg_ellipse, width=9, height=5)

ggsave(
  "figures/acropora/acr_pca_deg_ellipse.svg",
  acr_pca_deg_ellipse,
  width = 9,
  height = 5
)
...
vst_all_df <- cbind(
  gene = rownames(vst_all),
  as.data.frame(vst_all)
)

write.csv(vst_all_df,
          "data/acr_all_genes_vst_expression.csv",
          row.names = FALSE)
```